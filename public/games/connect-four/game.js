...
function renderBoard(state) {
  const board = document.getElementById('cfBoard');
  ...
  for (let c = 0; c < 7; c++) {
    ...
    btn.textContent = '▼';
    btn.disabled = !myTurn || !!state.winner;
  ...
  for (let r = 0; r < 6; r++) {
    ...
    for (let c = 0; c < 7; c++) {
      ...
      if (v === 0) cell.classList.add('p0');
      if (v === 1) cell.classList.add('p1');
      // Highlight last placed piece with glow
      if (state.lastMove && state.lastMove.row === r && state.lastMove.col === c && state.winner) {
...
function dropPiece(col) {
  if (!myTurn) return;
  socket.emit('make-move', { move: { col } });
}
...
function copyLink() {
  const url = `${location.origin}/games/connect-four/?room=${roomCode}`;
